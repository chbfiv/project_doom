...
//		anim.SetFloat("Direction", direction); 						// Animator側で設定している"Direction"パラメタにhを渡す
		...
		// 以下、キャラクターの移動処理
		velocity = new Vector3(0, 0, speed);		// 上下のキー入力からZ軸方向の移動量を取得
			...
			if(!anim.IsInTransition(0))
			{
				...
				if(useCurves){
					...
					if (Physics.Raycast(ray, out hitInfo))
					{
						...
						{
							controller.height = orgColHight - jumpHeight;			// 調整されたコライダーの高さ
							float adjCenterY = orgVectColCenter.y + jumpHeight;
							controller.center = new Vector3(0, adjCenterY, 0);	// 調整されたコライダーのセンター
						}
						else{
